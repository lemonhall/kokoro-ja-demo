...
**问题：** 安装 `pip install misaki[ja]` 时会下载一个很大的 UniDic 词典（约 400MB），这个词典的作用是什么？

**解答：**

**UniDic** 是 MeCab 的标准日文词典，包含约 **70万+ 词条**，是日文分词和语音合成的核心数据。

#### UniDic 词典的作用

1. **日文分词（Tokenization）**
   - 提供完整的日文词汇表
   - 用于 Viterbi 算法的最优路径搜索
   - 词频统计（cost 值），帮助消除歧义
...
3. **读音信息（Pronunciation）**
   - 假名标注（用于 G2P）
   - 重音信息

#### C 语言版本如何使用词典？

- **Python 版本：** 通过 MeCab 直接访问 UniDic
- **C 语言版本：** 需要提取为 TSV 格式的词典文件
...
# 2. 运行提取脚本
python generate_ja_dict.py

# 3. 生成的词典文件：
# extracted_data/ja/dict.txt
```
